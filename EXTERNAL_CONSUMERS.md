...
- изменить порядок или иерархию страниц

Вход:

- `page_id` — id перемещаемой страницы
- `target_page_id` — id страницы-ориентира
- `position` — `before`, `after` или `append`

`before` и `after` помещают страницу до или после целевой страницы под
того же родителя. `append` делает её последней дочерней страницей `target_page_id`.
